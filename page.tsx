...
async function createDefaultTasksForStage(
 ...
 setTasks((current) => [
  ...
  async function handleLeadCreated(newLead: Lead) {
    setLeads((current) => [newLead, ...current]);
    await createDefaultTasksForStage(newLead.id, newLead.etap || "nowy_lead");
                    ...
                    >
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={isDone}
                          onChange={() => void onToggleTaskStatus?.(task.id)}
                          style={{
                            width: 18,
                            height: 18,
                            marginTop: 4,
