...
export async function POST(request: NextRequest) {
  ...
  if (profile?.aktywne === false || !ALLOWED_ROLES.has(String(profile?.role || ""))) {
    return NextResponse.json({ error: "Brak uprawnień do dodania PDF z CRBR." }, { status: 403 });
  }
  ...
  if (!(file instanceof File)) {
    return NextResponse.json({ error: "Dodaj plik PDF pobrany z CRBR." }, { status: 400 });
  }
  if (file.size <= 0 || file.size > MAX_FILE_SIZE) {
    return NextResponse.json({ error: "Plik PDF jest pusty albo przekracza limit 25 MB." }, { status: 400 });
  }
  if (!isPdfFile(file)) {
    return NextResponse.json({ error: "Plik z CRBR musi być PDF-em." }, { status: 400 });
  }
  ...
  const register = await ensureAmlRegister(admin, client.id).catch((error) => {
    console.error("Nie udało się przygotować rejestru AML dla PDF z CRBR:", error);
    return null;
  });
  if (!register) {
  ...
  const originalFileName = file.name || "crbr.pdf";
  const storageName = `${Date.now()}-${sanitizeFileName(originalFileName)}`;
  const storagePath = `aml/${client.id}/crbr/${storageName}`;
  ...
  if (verificationError || !verification) {
    ...
    return NextResponse.json({ error: verificationError?.message || "Nie udało się zapisać PDF z CRBR." }, { status: 500 });
  }
...
function sanitizeFileName(value: string) {
  const normalized = value
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 120);
  return normalized.toLowerCase().endsWith(".pdf") ? normalized : `${normalized || "crbr"}.pdf`;
}
