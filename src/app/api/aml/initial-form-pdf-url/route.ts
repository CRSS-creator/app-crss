...
export async function POST(request: NextRequest) {
  ...
  if (profile?.aktywne === false || !ALLOWED_ROLES.has(String(profile?.role || ""))) {
    return NextResponse.json({ error: "Brak uprawnień do formularza wstępnego AML." }, { status: 403 });
  }

  const body = await request.json().catch(() => null) as { formId?: string } | null;
  if (!body?.formId) {
    return NextResponse.json({ error: "Brak formularza wstępnego AML." }, { status: 400 });
  }
  ...
  if (formError || !form?.completed_pdf_document_id) {
    return NextResponse.json({ error: "Nie znaleziono PDF formularza wstępnego AML." }, { status: 404 });
  }

  const { data: document, error: documentError } = await admin
    .from("klienci_dokumenty")
    .select("nazwa, sciezka")
    .eq("id", form.completed_pdf_document_id)
    .single();

  if (documentError || !document?.sciezka) {
    return NextResponse.json({ error: "Nie znaleziono dokumentu formularza wstępnego AML." }, { status: 404 });
  }

  const signedUrl = await admin.storage
    .from(CLIENT_DOCUMENTS_BUCKET)
    .createSignedUrl(document.sciezka, 10 * 60);
  ...
  return NextResponse.json({
    ...
    fileName: document.nazwa || "Formularz_wstepny_AML.pdf",
  });
}
