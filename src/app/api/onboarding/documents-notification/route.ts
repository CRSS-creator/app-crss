...
type DocumentsNotificationPayload = {
  clientId?: string;
};
...
function getWebhookUrl() {
  const webhookUrl = process.env.N8N_ONBOARDING_DOCUMENTS_WEBHOOK_URL?.trim();

  if (!webhookUrl) {
    return {
      webhookUrl: null,
      error: NextResponse.json(
        { error: "Brak konfiguracji wysyłki listy dokumentów. Uzupełnij N8N_ONBOARDING_DOCUMENTS_WEBHOOK_URL." },
        { status: 500 }
      ),
    };
  }
...
async function getAuthorizedUser(request: NextRequest): Promise<AuthorizedResult> {
  ...
  if (!ALLOWED_ROLES.has(role)) {
    return { admin: null, requesterId: null, role: null, error: NextResponse.json({ error: "Brak uprawnień do wysyłki listy dokumentów." }, { status: 403 }) };
  }
...
function fileUrl(fileName: string) {
  return `${APP_URL}/${encodeURIComponent(fileName)}`;
}

function buildAttachments(client: ClientWithCaregiver) {
  const legalForm = normalize(client.forma_prawna);
...
function buildNotificationHtml(client: ClientWithCaregiver) {
  ...
  const caregiverName = caregiver?.full_name || "opiekun księgowy";
...
<div style="margin:0;padding:0;background:#f6f8fb;font-family:Arial,sans-serif;color:#173b73;">
  <div style="max-width:640px;margin:0 auto;padding:28px 18px;">
    <div style="background:#ffffff;border:1px solid #c9d6e8;border-radius:18px;padding:28px;">
      ...
      <p style="margin:0 0 16px 0;">w załączeniu przesyłamy listę dokumentów potrzebnych do rozpoczęcia współpracy dla <strong>${clientName}</strong>.</p>
      <div style="background:#eef3fb;border:1px solid #c9d6e8;border-radius:14px;padding:18px;margin:24px 0;">
        <p style="margin:0 0 12px 0;font-weight:850;">Przekazanie dokumentów</p>
        <p style="margin:0 0 8px 0;">Dokumenty prosimy przesłać bezpośrednio do opiekuna księgowego:</p>
        <p style="margin:0;"><strong>${caregiverName}</strong><br><a href="mailto:${caregiverEmail}" style="color:#173b73;">${caregiverEmail}</a></p>
      </div>
...
export async function POST(request: NextRequest) {
  ...
  let payload: DocumentsNotificationPayload;
  try {
  ...
  if (auth.role === "accountant" && clientRecord.opiekun_id !== auth.requesterId) {
    return NextResponse.json({ error: "Możesz wysyłać listę dokumentów tylko dla swoich klientów." }, { status: 403 });
  }
  ...
  if (!caregiver?.email) {
    return NextResponse.json({ error: "Klient nie ma przypisanego opiekuna księgowego z adresem e-mail." }, { status: 400 });
  }

  const attachments = buildAttachments(clientRecord);
  if (attachments.length === 0) {
    return NextResponse.json({ error: "Nie udało się dobrać listy dokumentów do formy prawnej klienta." }, { status: 400 });
  }
  ...
  try {
    const response = await fetch(webhookConfig.webhookUrl, {
      ...
      body: JSON.stringify({
        event: "onboarding_documents_notification_requested",
        clientId: clientRecord.id,
        clientName: clientRecord.nazwa,
        clientNip: clientRecord.nip,
        recipientEmail: clientRecord.email,
        subject: "Lista dokumentów potrzebnych do rozpoczęcia współpracy",
