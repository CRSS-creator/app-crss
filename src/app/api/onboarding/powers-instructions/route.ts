...
type PowersInstructionsPayload = {
  clientId?: string;
};
...
function getWebhookUrl() {
  const webhookUrl = process.env.N8N_ONBOARDING_POWERS_WEBHOOK_URL?.trim();

  if (!webhookUrl) {
    return {
      webhookUrl: null,
      error: NextResponse.json(
        { error: "Brak konfiguracji wysyłki instrukcji. Uzupełnij N8N_ONBOARDING_POWERS_WEBHOOK_URL." },
        { status: 500 }
      ),
    };
  }
...
async function getAuthorizedUser(request: NextRequest): Promise<AuthorizedResult> {
  ...
  if (!ALLOWED_ROLES.has(role)) {
    return { admin: null, requesterId: null, role: null, error: NextResponse.json({ error: "Brak uprawnień do wysyłki instrukcji." }, { status: 403 }) };
  }
...
function buildInstructionHtml(client: ClientWithCaregiver) {
...
<div style="margin:0;padding:0;background:#f6f8fb;font-family:Arial,sans-serif;color:#173b73;">
  <div style="max-width:640px;margin:0 auto;padding:28px 18px;">
    <div style="background:#ffffff;border:1px solid #c9d6e8;border-radius:18px;padding:28px;">
      ...
      <p style="margin:0 0 16px 0;">w załączeniu przesyłamy instrukcje dotyczące nadania upoważnień do ZUS oraz US dla <strong>${clientName}</strong>.</p>
      <div style="background:#eef3fb;border:1px solid #c9d6e8;border-radius:14px;padding:18px;margin:24px 0;">
        <p style="margin:0 0 12px 0;font-weight:850;">Dane osoby upoważnianej</p>
        <p style="margin:0 0 8px 0;"><strong>Imię i nazwisko:</strong> Mateusz Marcinkowski</p>
        <p style="margin:0 0 8px 0;"><strong>PESEL:</strong> 00301800172</p>
        <p style="margin:0 0 8px 0;"><strong>Adres do korespondencji:</strong> ul. Szewska 1/5, 63-100 Śrem</p>
...
export async function POST(request: NextRequest) {
  ...
  let payload: PowersInstructionsPayload;
  try {
  ...
  if (auth.role === "accountant" && clientRecord.opiekun_id !== auth.requesterId) {
    return NextResponse.json({ error: "Możesz wysyłać instrukcje tylko dla swoich klientów." }, { status: 403 });
  }
  ...
  try {
    const response = await fetch(webhookConfig.webhookUrl, {
      ...
      body: JSON.stringify({
        event: "onboarding_powers_instructions_requested",
        clientId: clientRecord.id,
        clientName: clientRecord.nazwa,
        clientNip: clientRecord.nip,
        recipientEmail: clientRecord.email,
        subject: "Instrukcje nadania pełnomocnictw ZUS i US",
        html: buildInstructionHtml(clientRecord),
        attachments,
        authorizedPerson: {
          fullName: "Mateusz Marcinkowski",
          pesel: "00301800172",
          address: "ul. Szewska 1/5, 63-100 Śrem",
          phone: "600-950-940",
        },
