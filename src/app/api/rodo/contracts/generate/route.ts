...
import type { RodoProcessingContract } from "@/lib/rodoProcessingContractService";

type GenerateRodoContractRequest = {
  contract?: RodoProcessingContract;
};
...
export async function POST(request: NextRequest) {
  ...
  if (!templatePath || !existsSync(templatePath)) {
    return NextResponse.json(
      { error: "Brakuje szablonu DOCX Umowa_powierzenia_danych_N8N.docx w katalogu templates." },
      { status: 500 }
    );
  }

  const workDir = await mkdtemp(path.join(tmpdir(), "crss-rodo-contract-"));
  ...
  try {
    ...
    console.error("Błąd generowania umowy powierzenia z DOCX:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Nie udało się wygenerować umowy powierzenia z DOCX." },
      { status: 500 }
    );
  } finally {
...
function resolveTemplatePath() {
  ...
  const candidates = [
    "Umowa_powierzenia_danych_N8N.docx",
    "Umowa powierzenia danych N8N.docx",
...
function buildTemplateFields(contract: RodoProcessingContract) {
  const accountingNumber = contract.crm_umowy?.numer_umowy || "";

  return {
    numer: contract.numer_umowy || "",
    numer_umowy: contract.numer_umowy || "",
    numer_umowy_ksiegowej: accountingNumber,
    umowa_ksiegowa: accountingNumber,
    nazwa: contract.nazwa_klienta || "",
    firma: contract.nazwa_klienta || "",
    nazwa_firmy: contract.nazwa_klienta || "",
    nazwa_klienta: contract.nazwa_klienta || "",
    klient: contract.nazwa_klienta || "",
    kontrahent: contract.nazwa_klienta || "",
    siedziba: contract.siedziba || "",
    adres: contract.siedziba || "",
    nip: contract.nip || "",
    reprezentant: contract.reprezentant || "",
    osoba_reprezentujaca: contract.reprezentant || "",
    zakres: contract.zakres_powierzenia || "",
...
function buildGeneratedContractFileName(contractNumber: string | null, contractorName: string | null) {
  ...
  return `umowa_powierzenia_${numberPart}_${contractorPart}.pdf`;
}
