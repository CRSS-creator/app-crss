alter table public.onboarding_etapy
  drop constraint if exists onboarding_etapy_etap_check;

alter table public.onboarding_etapy
  add constraint onboarding_etapy_etap_check
  check (etap in ('contract', 'rodo', 'aml', 'client_card', 'powers', 'wfirma', 'drive', 'recurring'));

insert into public.onboarding_etapy (klient_id, etap, status)
select distinct existing.klient_id, 'client_card', 'do_wykonania'
from public.onboarding_etapy existing
where not exists (
  select 1
  from public.onboarding_etapy current_stage
  where current_stage.klient_id = existing.klient_id
    and current_stage.etap = 'client_card'
);
...
begin
  ...
  if not was_already_started then
    ...
    select
      ...
      'Umowa ' || coalesce(selected_contract.numer_umowy, 'bez numeru') || ' dla klienta ' || selected_contract.nazwa_klienta || ' została oznaczona jako podpisana i przekazana do onboardingu.',
      'high',
      'crm_umowy',
