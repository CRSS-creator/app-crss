...
begin
  ...
  if not was_already_started then
    ...
    select
      'onboarding_admin_tasks',
      'Nowy klient w onboardingu',
      'Klient ' || coalesce(selected_contract.nazwa_klienta, 'bez nazwy') || ' został przekazany do onboardingu. Po stronie administracji: AML, pełnomocnictwa, utworzenie konta wFirma oraz karta klienta biura rachunkowego, jeżeli dotyczy.',
      'high',
      'crm_umowy',
      selected_contract.id,
      profile.id,
