alter table public.faktury
  add column if not exists kategoria text not null default 'standardowa';
...
alter table public.faktury
  ...
  check (kategoria in ('standardowa', 'dodatkowa'));

create index if not exists faktury_kategoria_idx on public.faktury(kategoria);
