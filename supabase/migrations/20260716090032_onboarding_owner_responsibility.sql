...
begin
  ...
  if not was_already_started then
    ...
    select
      ...
      'Klient ' || coalesce(selected_contract.nazwa_klienta, 'bez nazwy') || ' został przekazany do onboardingu. Po stronie ownera: AML, utworzenie konta wFirma oraz karta klienta biura rachunkowego, jeżeli dotyczy.',
      'high',
      'crm_umowy',
      selected_contract.id,
      profile.id,
