...
add column if not exists vat_okres_rozliczeniowy text;

alter table public.klienci disable trigger trg_prevent_unauthorized_client_updates;

update public.klienci
set vat_okres_rozliczeniowy = 'miesieczny'
where vat_okres_rozliczeniowy is null;

alter table public.klienci enable trigger trg_prevent_unauthorized_client_updates;

alter table public.klienci
alter column vat_okres_rozliczeniowy set default 'miesieczny',
alter column vat_okres_rozliczeniowy set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'klienci_vat_okres_rozliczeniowy_check'
      and conrelid = 'public.klienci'::regclass
  ) then
    alter table public.klienci
    add constraint klienci_vat_okres_rozliczeniowy_check
    check (vat_okres_rozliczeniowy in ('miesieczny', 'kwartalny'));
  end if;
end;
$$;
...
begin
  ...
  where obligation.rozliczenie_id = settlement.id
    ...
    and (
      ...
      or (obligation.typ = 'CIT' and coalesce(client.forma_opodatkowania, '') <> 'CIT')
...
begin
  for settlement_period in
    select distinct okres from public.rozliczenia_miesieczne
  loop
    perform public.ensure_tax_obligations(settlement_period);
  end loop;
end;
$$;
