...
as $$
declare
  cursor_date date := input_date;
  remaining_days integer := greatest(business_days, 0);
begin
  while remaining_days > 0 loop
    cursor_date := cursor_date - 1;

    if extract(isodow from cursor_date) between 1 and 5 then
      remaining_days := remaining_days - 1;
    end if;
  end loop;

  while extract(isodow from cursor_date) in (6, 7) loop
    cursor_date := cursor_date - 1;
  end loop;
...
begin
  ...
  with contract_dates as (
    ...
    where contract_dates.due_date between public_due_date and public_due_date + (greatest(public_days_ahead, 0) + 7)
      and public.subtract_business_days(contract_dates.due_date, greatest(public_days_ahead, 0)) = public_due_date
  ...
  select
    ...
    jsonb_build_object(
      ...
      'business_days_ahead', public_days_ahead,
...
revoke all on function public.subtract_business_days(date, integer) from public;
grant execute on function public.subtract_business_days(date, integer) to authenticated;
