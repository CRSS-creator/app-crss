with ranked_due_notifications as (
  select
    id,
    row_number() over (
      partition by type, related_table, related_id, recipient_id
      order by created_at asc, id asc
    ) as duplicate_rank
  from public.powiadomienia
  where type in ('task_due_today', 'recurring_task_due_today')
    and related_table in ('zadania', 'zadania_cykliczne_realizacje')
    and related_id is not null
    and recipient_id is not null
)
delete from public.powiadomienia notification
using ranked_due_notifications ranked
where notification.id = ranked.id
  and ranked.duplicate_rank > 1;

create unique index if not exists powiadomienia_due_task_unique_idx
  on public.powiadomienia (type, related_table, related_id, recipient_id)
  where type in ('task_due_today', 'recurring_task_due_today')
    and related_table in ('zadania', 'zadania_cykliczne_realizacje')
    and related_id is not null
    and recipient_id is not null;
